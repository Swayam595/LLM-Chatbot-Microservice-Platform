...
def health_check(app_config: AppConfig = Depends(get_app_config)) -> dict:
    ...
    conversation_service_health = _check_conversation_service_health(app_config, "health")
    if conversation_service_health.status_code != 200:
        raise HTTPException(status_code=500, detail= {
            "status": "error",
            "detail": conversation_service_health.json()
        })
...
def health_check_all(app_config: AppConfig = Depends(get_app_config)) -> dict:
    ...
    conversation_service_health = _check_conversation_service_health(app_config, "health/all")
...
def _check_conversation_service_health(app_config: AppConfig, health_endpoint: str) -> bool:
    """Check the health of the conversation service"""
    try:
        conversation_service_health_check_url = f"{app_config.CONVERSATION_SERVICE_URL}/{health_endpoint}"
